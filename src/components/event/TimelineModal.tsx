...
const TimelineModal: React.FC<TimelineModalProps> = ({ isOpen, onClose }) => {
  ...
  const generateCompressedTimeScale = (segments: TimelineSegment[]): TimeScale[] => {
    ...
    segments.forEach(segment => {
      ...
      // 收集该segment内的所有刻度点
      const segmentTicks = new Map<number, TimeScale>();

      // 首先检查今天是否在这个segment内
      if (today >= segment.start && today <= segment.end) {
        const segmentProgress = (today.getTime() - segment.start.getTime()) / (segment.end.getTime() - segment.start.getTime());
        const position = segment.startPosition + (segment.endPosition - segment.startPosition) * segmentProgress;

        segmentTicks.set(today.getTime(), {
          date: new Date(today),
          position,
          isMonth: true, // 使用 isMonth 标记今天，以显示较粗的线
          isWeek: false,
      ...
      // 按时间排序并添加到scale中
      const sortedTicks = Array.from(segmentTicks.entries())
        .sort((a, b) => a[0] - b[0])
        .map(entry => entry[1]);

      scale.push(...sortedTicks);
    });
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="z-1000 fixed inset-0 flex justify-center items-center bg-black/40 p-4"
          onClick={onClose}
        >
          <motion.div
            ...
            className="flex flex-col bg-surface shadow-card-hover border border-line rounded-xl w-225 max-w-[95vw] max-h-[90vh] overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="relative flex justify-between items-center bg-surface-2 px-10 py-8 pb-6 border-line border-b">
              <h2 className="flex items-center gap-3 m-0 font-bold text-ink text-2xl">
                {loc("EventTimeline", "活动时间轴")}
              </h2>
              <button
                className="flex justify-center items-center bg-surface hover:border-danger/50 p-3 border border-line rounded-md w-12 h-12 text-ink-3 hover:text-danger text-xl transition-colors duration-200 cursor-pointer"
                onClick={onClose}
              >
                ✕
              </button>
