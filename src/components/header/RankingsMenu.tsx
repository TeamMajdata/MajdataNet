...
export default function RankingsMenu() {
  ...
  return (
    <div className="inline-block relative" ref={containerRef}>
      <button
        className={`flex items-center gap-2 px-5 py-3 text-white/85 no-underline rounded-lg font-medium text-sm whitespace-nowrap relative bg-none border-none cursor-pointer ${isOpen
      ...
      <Dropdown isOpen={isOpen} onClose={() => setIsOpen(false)} position="left" className="w-full" containerRef={containerRef}>
        ...
        <Link to="/ranking/user" className={DROPDOWN_ITEM}>
          <span className="text-sm">{loc('UserRankingTitle')}</span>
        </Link>
        <Link to="/ranking/mmfc" className={DROPDOWN_ITEM}>
