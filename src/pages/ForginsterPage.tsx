...
function LoginTab() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formData.set(
        "rememberMe",
        (formData.get("rememberMe") != null).toString(),
      );
      ...
      if (response.status !== 200) {
        ...
        switch (rsp.code) {
          ...
          case retCode.CODE_LOGIN_FAILED_PENDING_VERIFCATION:
            toast.error(
              loc("[Login]PendingVerifcation", "账户尚未激活，请查收邮件"),
            );
            break;
        ...
        if (response.status === 400) {
          switch (rsp.code) {
            ...
            case retCode.CODE_INVALID_VALUE:
              toast.error(
                loc("InvalidUsernameOrPassword", "用户名或密码格式错误"),
              );
              break;
      ...
      <form className="flex flex-col gap-6" onSubmit={onSubmit}>
        ...
        <div className="flex flex-col gap-2 scale-75 md:scale-100 origin-top-left">
          ...
          <div
            ...
            data-sitekey="0x4AAAAAACAEyA1EhHmEDS0o"
          ></div>
        </div>
        <button
          className="relative bg-linear-to-r from-[#5C8DC1] hover:from-[#4A7DAF] to-[#4A7DAF] hover:to-[#3A6D9F] disabled:opacity-50 hover:shadow-[0_10px_25px_rgb(92_141_193/30%)] mt-2 p-4 border-none rounded-full overflow-hidden font-semibold text-white transition-all hover:-translate-y-0.5 active:translate-y-0 cursor-pointer disabled:cursor-not-allowed"
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!hasOtp) {
        ...
        toast.success(loc("ResetEmailSent", "重置密码邮件已发送"), {
          autoClose: false,
        });
      } else {
        if (
        ...
        toast.success(loc("ResetPasswordSuccess", "密码重置成功"), {
          autoClose: false,
        });
      }
    } finally {
      ...
      <form className="flex flex-col gap-6" onSubmit={onSubmit}>
        ...
        >
          <span className="z-10 relative">
            {hasOtp
              ? loc("ResetPasswordButton", "确认重置密码")
              : loc("SendVerificationEmail", "发送验证邮件")}
          </span>
        </button>
      </form>
    </div>
