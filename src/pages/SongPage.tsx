    ...
    <div className="bg-transparent p-6">
      <section className="gap-8 grid grid-cols-1 lg:grid-cols-[320px_1fr] mb-10">
        <div className="flex justify-center items-start">
          <div className="shadow-card w-48 md:w-64 h-48 md:h-64 hover:scale-105 transition-transform duration-300">
            <CoverPic id={o.id} />
          </div>
        </div>
        ...
        <div className="flex flex-col justify-between gap-3">
          ...
          >
            <Tooltip content={loc('SearchForTitle') || '点击搜索该歌曲'}>
              <h1
                className="inline-block hover:text-primary px-5 py-2.5 rounded-lg font-black text-ink text-4xl md:text-5xl text-center tracking-tight transition-colors hover:-translate-y-0.5 duration-300 cursor-pointer"
                id={o.id}
                onClick={() => {
            ...
            <Tooltip content={loc('SearchForArtist') || '点击搜索该艺术家'}>
              <div className="font-medium text-ink-2 text-xl md:text-2xl text-center">
                <span
                  className="inline-block hover:bg-surface-2 px-3 py-1 rounded-md hover:text-primary transition-colors hover:-translate-y-0.5 duration-300 cursor-pointer"
                  onClick={() => {
          ...
          <div className="flex flex-col items-start gap-3">
            <h3 className="m-0 font-bold text-ink text-sm uppercase tracking-[0.05em]">All Difficulties</h3>
            <SongDifficultyLevels
              levels={o.levels}
              songid={o.id}
              isPlayer={true}
      ...
      <div className="lg:items-start gap-8 grid grid-cols-1 lg:grid-cols-[320px_1fr]">
        <aside className="flex flex-col gap-4">
          ...
          <div className="flex md:flex-row flex-col lg:flex-col gap-4 md:gap-4 lg:gap-4">
            <Tooltip content={o.uploader + '@' + o.designer}>
              <Link to={'/space?id=' + o.uploader} className="inline-flex flex-1 items-center gap-3.5 bg-surface border border-line hover:border-primary/50 shadow-card px-3 py-1.5 rounded-lg text-ink hover:text-primary no-underline transition-colors hover:-translate-y-0.5 duration-300">
                <img
                  className="shadow-card border border-line rounded-full w-9 min-w-9 h-9 min-h-9 aspect-square transition-all duration-300 shrink-0"
                  src={endpoints.account.icon(o.uploader)}
                  alt={o.uploader}
                />
                <div className="flex flex-col items-start gap-1">
                  <span className="font-semibold text-ink text-base leading-[1.3]">{o.uploader}</span>
                  <span className="font-normal text-ink-3 text-sm leading-[1.3]">{o.designer}</span>
                </div>
              </Link>
            </Tooltip>

            <div className="flex-1 bg-surface border border-line shadow-card p-5 rounded-lg transition-all duration-300">
              <h3 className="mb-3 font-bold text-ink text-sm uppercase tracking-wider">
                {loc('Tags') || '标签'}
              </h3>
          ...
          <div className="flex flex-col gap-2 bg-surface border border-line shadow-card p-5 rounded-lg transition-all duration-300">
            <button
              className="bg-surface border border-line hover:border-primary hover:text-primary shadow-card rounded-lg w-full h-11 font-semibold text-ink-2 text-base transition-colors"
          ...
          {/* ID/HASH/点赞面板 */}
          <div className="relative flex flex-col bg-surface border border-line shadow-card p-5 rounded-lg overflow-hidden transition-all duration-300">
            ...
            <div className="my-3 border-line border-t"></div>
        ...
        <main className="flex flex-col gap-8">
          {isLoadMajdata ? (
            ...
            <button
              className="bg-surface border border-line hover:border-primary shadow-card p-2.5 rounded-lg w-full aspect-square font-bold text-base transition-colors"
