...
export default function SongPage() {
  ...
  return (
    <PageLayout>
      <div
        className="-z-10 fixed inset-0 bg-cover bg-top-left blur-[10px] brightness-60"
...
function SongDetailsContainer({
  id,
  data,
}: SongDetailsContainerProps & { data: SongSummary }) {
  return (
...
function SongInfo({ data }: { id: string; data: SongSummary }) {
  ...
  const OnDownloadClick =
    (params: { id: string; title: string }) => async () => {
      ...
      <section className="gap-8 flex flex-col mb-10">
        ...
        <div className="flex flex-col justify-between gap-3">
          ...
          <div className="flex flex-col items-start gap-3">
            <h3 className="m-0 font-bold text-gray-700 text-sm uppercase tracking-[0.05em]">
              All Difficulties
            </h3>
            <SongDifficultyLevels
              levels={o.levels}
      ...
      <div className="lg:items-start gap-8 grid grid-cols-1 lg:grid-cols-[320px_1fr]">
        <aside className="flex flex-col gap-4">
          ...
          <div className="flex md:flex-row flex-col lg:flex-col gap-4 md:gap-4 lg:gap-4">
            <Tooltip content={o.uploader + "@" + o.designer}>
              <Link
                to={"/space?id=" + o.uploader}
                className="inline-flex flex-1 items-center gap-3.5 bg-black/5 hover:bg-black/10 shadow-sm border border-gray-200 hover:border-[#5C8DC1]/30 rounded-xl text-gray-700 hover:text-[#5C8DC1] no-underline transition-all hover:-translate-y-0.5 duration-300"
              >
                <img
                  className="shadow-sm border-2 border-gray-200 rounded-full w-9 min-w-9 h-9 min-h-9 aspect-square transition-all duration-300 shrink-0"
                  src={endpoints.account.icon(o.uploader)}
                  alt={o.uploader}
                />
                <div className="flex flex-col items-start gap-1">
                  <span className="font-semibold text-gray-800 text-base leading-[1.3]">
                    {o.uploader}
                  </span>
                  <span className="font-normal text-gray-500 text-sm leading-[1.3]">
                    {o.designer}
                  </span>
                </div>
              </Link>
            ...
            <div className="flex-1 bg-white shadow-sm border border-gray-200 p-5 rounded-2xl transition-all duration-300">
              <h3 className="mb-3 font-bold text-gray-700 text-sm uppercase tracking-wider">
                {loc("Tags") || "标签"}
              </h3>
                  ...
                  <>
                    {o.tags.map((tag, index) => (
                      <Tooltip content={loc("SearchForTag")} key={index}>
                        <span
                          className="inline-flex items-center bg-[linear-gradient(135deg,rgb(92_141_193/0.3),rgb(92_141_193/0.2))] hover:bg-[linear-gradient(135deg,rgb(92_141_193/0.5),rgb(92_141_193/0.4))] hover:shadow-[0_4px_12px_rgb(92_141_193/0.3)] px-3 py-1 border border-[rgb(92_141_193/0.4)] hover:border-[rgb(92_141_193/0.7)] rounded-full font-medium text-[rgb(92_141_193/0.9)] hover:text-[#5C8DC1] text-xs transition-all hover:-translate-y-0.5 duration-300 cursor-pointer"
                          onClick={() => {
          ...
          <div className="flex flex-col gap-2 bg-white shadow-sm border border-gray-200 p-5 rounded-2xl transition-all duration-300">
            <button
              className="bg-gray-100 hover:bg-gray-200 border border-gray-300 rounded-xl w-full h-11 font-bold text-gray-700 text-base transition-all"
          ...
          {/* ID/HASH/点赞面板 */}
          <div className="relative flex flex-col bg-white shadow-sm border border-gray-200 p-5 rounded-2xl overflow-hidden transition-all duration-300">
            <div className="gap-y-3 grid grid-cols-[80px_1fr] text-sm">
              ...
              <code
                className="bg-gray-100 px-2 py-0.5 rounded w-fit font-mono text-gray-600 text-xs text-center break-all"
                style={{ cursor: "pointer" }}
                title="点击复制"
                onClick={() => {
              ...
              <span className="pl-1.75 w-fit text-gray-600 text-xs text-center">
        ...
        <main className="flex flex-col gap-8">
          {isLoadMajdata ? (
            ...
            <button
              className="bg-gray-100 hover:bg-gray-200 border border-gray-300 rounded-xl w-full aspect-square font-bold text-base transition-all"
...
function MajdataView({ data }: { id: string; data: SongSummary }) {
  ...
  return (
    <div className="w-full">
      <Majdata
        songid={o.id}
        chartRoot={endpoints.maichart.prefix("")}
