    ...
    <PageLayout className="flex justify-center items-center min-h-[60vh]">
      ...
      >
        ...
        <div className="mb-8">
          ...
          >
            {i18n("not-found/NotFoundPage.Title", '页面未找到')}
          </p>
          <p className="text-[#b8b8b8] text-[1rem] leading-relaxed">
            {i18n("not-found/NotFoundPage.Description", '抱歉，您访问的页面不存在或已被移除。')}
          </p>
        </div>
        ...
        <div className="bg-[rgba(167,139,250,0.1)] mb-8 p-4 border border-[rgba(167,139,250,0.3)] rounded-lg">
          <p className="text-[#d4c5f9] text-[0.95rem]">
            {i18n("not-found/NotFoundPage.Redirect", `${countdown} 秒后自动返回首页...`).replace('${countdown}', String(countdown))}
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          ...
          >
            {i18n("not-found/NotFoundPage.BackHome", '返回首页')}
          </Link>
          <button
            onClick={() => navigate(-1)}
          ...
          >
            {i18n("not-found/NotFoundPage.GoBack", '返回上一页')}
          </button>
        </div>
      </div>
    </PageLayout>
